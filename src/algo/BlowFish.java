...
{
    static final String NAME = "BlowFish";
    static final boolean IN = true, OUT = false;
    static final boolean DEBUG = BlowFish.DEBUG;
	...
	{
			int[][] Ke = (int[][]) ((Object[]) sessionKey)[0]; 		int ROUNDS = Ke.length - 1;
		int[] Ker = Ke[0];
    ...
    blockDecrypt (byte[] in, int inOffset, Object sessionKey) {
        int[][] Kd = (int[][]) ((Object[]) sessionKey)[1];        int ROUNDS = Kd.length - 1;
        int[] Kdr = Kd[0];
        ...
        int a0, a1, a2, a3;
        for (int r = 1; r < ROUNDS; r++) {                      Kdr = Kd[r];
            a0   = (T5[(t0 >>> 24) & 0xFF] ^
                    T6[(t3 >>> 16) & 0xFF] ^
                    T7[(t2 >>>  8) & 0xFF] ^
                    T8[ t1         & 0xFF]  ) ^ Kdr[0];
        ...
        byte[] result = new byte[16];         Kdr = Kd[ROUNDS];
        int tt = Kdr[0];
